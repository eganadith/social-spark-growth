...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen pt-24 pb-28 md:pb-20 px-4">
      <div className="container mx-auto max-w-4xl">
        {/* Header */}
        <div className="relative overflow-hidden rounded-3xl border border-white/[0.08] bg-gradient-to-br from-card via-card to-primary/[0.07] p-6 sm:p-8 mb-8 shadow-[0_8px_40px_-12px_hsla(280,70%,20%,0.45)]">
          ...
          <div className="relative flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2 opacity-0 animate-fade-in-up [animation-delay:0.12s]">
              <Button asChild variant="hero" size="sm" className="rounded-xl shadow-lg">
                <Link to="/order">New order</Link>
              </Button>
              <Button variant="outline" size="sm" className="rounded-xl border-white/15 bg-background/40 backdrop-blur-sm" onClick={() => void signOut()}>
                Sign out
              </Button>
            </div>
          </div>
        ...
        {loadingData ? (
          ...
          <>
            ...
            {/* Referral */}
            <section
              className="rounded-3xl border border-white/[0.1] bg-card/70 p-6 sm:p-8 shadow-card mb-8 opacity-0 animate-fade-in-up backdrop-blur-xl"
              style={{ animationDelay: "0.14s" }}
            >
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl gradient-bg text-white shadow-lg">
                    <Share2 className="h-6 w-6" />
                  </div>
                  <div>
                    <h2 className="text-lg font-bold">Your referral link</h2>
                    <p className="text-sm text-muted-foreground">Friends who purchase count toward your milestones.</p>
                  </div>
                </div>
              ...
              {/* Milestone stepper */}
              <div className="mb-6 rounded-2xl border border-border/80 bg-muted/30 p-4 sm:p-5">
                <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-4">Milestone path</p>
                  ...
                  {REWARD_MILESTONES.map((m) => {
                    ...
                    return (
                      <div key={m.code} className="flex flex-col items-center text-center">
                        <div
                          className={cn(
                            "flex h-9 w-9 sm:h-11 sm:w-11 items-center justify-center rounded-full border-2 transition-all",
                            unlocked
                              ? "border-transparent gradient-bg text-white shadow-md sm:scale-105"
                              : "border-border bg-background/80 text-muted-foreground",
                          )}
                        >
                          {unlocked ? <CheckCircle2 className="h-4 w-4 sm:h-5 sm:w-5" /> : <Lock className="h-3.5 w-3.5 sm:h-4 sm:w-4" />}
                        </div>
                        <span className="mt-1.5 text-[10px] sm:text-xs font-bold text-foreground leading-tight">{m.referrals} ref</span>
                        <span className="text-[9px] sm:text-[10px] text-muted-foreground font-mono leading-tight">{m.code}</span>
                      </div>
                    );
                  })}
                </div>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-stretch">
                  <div className="relative flex-1 min-w-0 rounded-xl border border-white/10 bg-background/50 ring-offset-background focus-within:ring-2 focus-within:ring-primary/40 focus-within:ring-offset-2 focus-within:ring-offset-background transition-shadow">
                    <div className="flex items-center gap-2 px-4 py-3">
                      ...
                      <p className="text-sm font-mono break-all text-foreground/90 leading-relaxed">{shareUrl}</p>
                    </div>
                  </div>
                  <Button
                    type="button"
                    variant="secondary"
                    className="gap-2 shrink-0 rounded-xl h-12 sm:h-auto sm:px-6"
                    onClick={copyShare}
                  >
            ...
            {/* Orders */}
            <section
              className="rounded-3xl border border-white/[0.1] bg-card/70 p-6 sm:p-8 shadow-card opacity-0 animate-fade-in-up backdrop-blur-xl"
